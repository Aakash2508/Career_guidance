...
import { HouseDoor, InfoCircle, StarFill, Envelope } from "react-bootstrap-icons";
import React, { useState, useEffect, useRef } from "react";
import axios from "axios";
import { Modal, Form, ListGroup, InputGroup,Dropdown,Row,Col } from "react-bootstrap";
...
function Layout({ children }) {
    ...
    return (
      ...
      <Navbar.Collapse id="basic-navbar-nav">
        <Nav className="ms-auto gap-3 align-items-center">
          ...
          {/* Mega Dropdown controlled via state */}
  ...
  <div
    ...
    style={{
      width: "320px",
      height: "calc(100vh - 140px)", // Adjust height based on your header/footer
      position: "fixed",
  ...
  >
    <Card
  className="shadow-sm quick-links-card"
  style={{
    borderRadius: "16px",
    border: "none",
    marginTop: "20px",
...
>
  <Card.Body>
    <br />
    <Card.Title className="text-left mb-4 fs-2 fw-bold" >Quick Link</Card.Title><br/>
    <Nav className="flex-column gap-4">
      <Nav.Link
        className="quick-link-item d-flex align-items-center gap-2 fw-semibold fs-4"
        onClick={() => document.getElementById("home-section")?.scrollIntoView({ behavior: "smooth" })}
      >
        <HouseDoor size={28} /> Home
      </Nav.Link>
      <Nav.Link
        className="quick-link-item d-flex align-items-center gap-2 fw-semibold fs-4"
        onClick={() => document.getElementById("about-section")?.scrollIntoView({ behavior: "smooth" })}
      >
        <InfoCircle size={28} /> About Us
      </Nav.Link>
      <Nav.Link
        className="quick-link-item d-flex align-items-center gap-2 fw-semibold fs-4"
        onClick={() => document.getElementById("features-section")?.scrollIntoView({ behavior: "smooth" })}
      >
        <StarFill size={28} /> Features
      </Nav.Link>
      <Nav.Link
        className="quick-link-item d-flex align-items-center gap-2 fw-semibold fs-4"
        onClick={() => document.getElementById("contact-section")?.scrollIntoView({ behavior: "smooth" })}
      >
        <Envelope size={28} /> Contact
      </Nav.Link>
    </Nav>
  </Card.Body>
</Card>
